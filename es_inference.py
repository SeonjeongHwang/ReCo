...
def parse_args():
    ...
    parser.add_argument('--strategy', type=str, help="es.cot")
...
def get_examples(data_file):
    ...
    for data in json.load(open(data_file)):        
        ...
        input_prompt = prompt_template.replace("{ passage }", passage).replace("{ statement }", statement).replace("{ factuality }", factuality) 
...
def extract_output(response):
    ...
    labels = ["Insufficient", "Single", "Inter"]
    for label in labels:
        if response.strip().split("\n")[0] == label.lower():
            return label
        
    pattern = r'answer:\s+(single|inter|insufficient)'
...
def main():
    ...
    exp_dir = f"{args.output_dir}/es_output/{args.model_nickname}-{args.strategy}-{args.demo}shot"
    if args.num_trial == 1:
        exp_dir += "-greedy"
    else:
